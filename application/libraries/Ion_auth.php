...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array(),$interests = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ion_auth_model->register($username, $password, $email, $additional_data, $group_ids,$interests);
			if ($id !== FALSE)
			{
...
public function get_user_name()
{
	...
	* @return
	*/
	public function is_client($id=false)
	{
		$this->ion_auth_model->trigger_events('is_client');

		$client_group = $this->config->item('client_group', 'ion_auth');

		return $this->in_group($client_group, $id);
	}
	public function is_staff($id=false)
	{
		$this->ion_auth_model->trigger_events('is_staff');

		$staff_group = $this->config->item('staff_group', 'ion_auth');

		return $this->in_group($staff_group, $id);
	}
	/**
	 * in_group
	 *
