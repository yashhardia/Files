...
class Ion_auth_model extends CI_Model
{
    ...
    /**
     * caching of users and their categories
     *
     * @var array
     **/
    public $_cache_user_in_cat = array();
    /**
     * caching of groups
     *
     * @var array
    ...
    public function change_password($identity, $old, $new)
    {
		...
		if($old_password_matches==FALSE)
		{
			$this->set_error('current_password_does_not_match');
			return FALSE;
		}
     ...
     * @return bool
     * @author Mathew
     *public function register($username, $password, $email, $additional_data = array(), $groups = array(),$interests = array())
     **/
    ...
    public function category($id = NULL)
    {
        ...
        if (isset($id)) {
            $this->where($this->tables['categories'] . '.category_id', $id);
        }
        $this->limit(1);
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login')->where($this->identity_column, get_cookie($this->config->item('identity_cookie_name', 'ion_auth')))->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))->limit(1)->order_by('id', 'desc')->get($this->tables['users']);
